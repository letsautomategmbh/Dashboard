...
1. Create `Widgets/YourWidget.php` implementing `Widgets/Widget.php`'s two static methods:
   - `isAvailable(User $user): bool` — gate on the source module actually being active (`Modules\Dashboard\Support\ModuleCheck::x()`, backed by `Route::has()` on one of that module's own routes — not `class_exists()`, since a deactivated module's classes stay autoload-able even though its routes/hooks never boot) and, if relevant, on `$user->isAdmin()`.
...
- One `dashboard_widgets` row per (user, widget instance): `widget_key`, `size`, `position`, `config`. Reordering rewrites every row's `position` in one request (KnowledgeBase's whole-array pattern), rather than reconciling incremental deltas.
...
- One instance of a given widget per user (`unique(user_id, widget_key)`) — none of the widgets need multiple simultaneous instances, so this is a simplification, not a technical ceiling.
...
Every cross-module widget is soft-optional: `module.json`'s `"requires": []` is intentionally empty, and each such widget's `isAvailable()` checks the source module is genuinely **active** — not merely installed — via `ModuleCheck::x()` before offering itself in the "add widget" picker or rendering on an existing board. A `class_exists()` check alone is not sufficient here: a deactivated module's PHP classes stay perfectly autoload-able (Composer's classmap doesn't care about FreeScout's own active/inactive DB flag), so `ModuleCheck` instead probes `Route::has()` on a stable route from that module — routes only get registered when a module's service provider actually boots, which only happens while FreeScout considers it active.
